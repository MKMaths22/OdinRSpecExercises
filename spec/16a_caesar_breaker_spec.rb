...
describe CaesarBreaker do
  ...
  describe '#save_decrypted_messages' do
    ...
    context 'when the directory does not exist' do
      ...
      before do
        ...
        allow(phrase).to receive(:create_filename).and_return('new_filename')
      end
      ...
      it 'sends message to create a file' do
        expect(File).to receive(:open).with("16_cipher/new_filename.yaml", 'w')
        phrase.save_decrypted_messages
      end
    end
  ...
  describe '#save_to_yaml' do
    ...
    it 'dumps to yaml' do
      expect(YAML).to receive(:dump).with('message' => phrase.instance_variable_get(:@message), 'decrypted_messages' => phrase.instance_variable_get(:@decrypted_messages))
      phrase.save_to_yaml
    end
  end
end
