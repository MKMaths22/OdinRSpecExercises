...
describe NumberGame do
  ...
  describe '#player_turn' do
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        ...
        next_letter = 'k'
        valid_input = '3'
        allow(game_loop).to receive(:player_input).and_return(letter, next_letter, valid_input)
      end
  ...
  describe '#final_message' do
    ...
    context 'when count is 4 and over' do
      subject(:game) { described_class.new(6, '6', 4) }
      # remove the 'x' before running this test
      it 'outputs correct phrase' do
        output_phrase = "That was hard. It took you 4 guesses!\n"
        expect{ game.final_message }.to output(output_phrase).to_stdout
      end
    end
  end
end
